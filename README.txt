...
= 2.6.6 =
* Fixed a PHP Error Notice for $user_id that could cause order updates to fail for guest checkout after payment has been received.

= 2.6.5 =
* Changed the order description to use get_order_number() instead of the actual order ID

= 2.6.4 =
* Added a filter to allow disabling of SSL verify for servers who are having issues connecting to wpFortify via wp_remote_post
